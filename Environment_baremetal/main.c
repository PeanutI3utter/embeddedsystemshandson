...
int main(void) {
	...
	RCC->APB1ENR |= RCC_APB1ENR_TIM3EN; // APB peripheral clock enable for TIM3
	TIM3->SMCR &= ~TIM_SMCR_SMS; // Disable Slave mode that the counter is clocked by the internal clock
	TIM3->CR1 &= ~TIM_CR1_DIR; // set upcounting mode
	TIM3->PSC = (unsigned short)80; // set prescaler to 480 so that counter frequency is lowered to 100kHz
	TIM3->ARR = 50000; // 50000 clocks are necessairy to create an overflow every 0.5s
