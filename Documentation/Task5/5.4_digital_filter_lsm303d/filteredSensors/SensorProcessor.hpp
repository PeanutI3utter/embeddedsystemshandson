...
public:
  SensorProcessor(Board *board) {
    ...
    // Sleep for first sensor reading
    ...
    // Fill Filter for offset
    for (int i = 0; i < SAMPLEFILTER_TAP_NUM; i++) {
      putSensorData();
    }

    // Calculate offset
    x.OFFSET = 0 - SampleFilter_get(&x);
    y.OFFSET = 0 - SampleFilter_get(&y);
    z.OFFSET = -9.81 - SampleFilter_get(&z);
  };

  void readAccelerometer(char msg[]) {
    axis axis_data;

    axis_data.x = SampleFilter_get(&x);
    axis_data.y = SampleFilter_get(&y);
    axis_data.z = SampleFilter_get(&z);
  ...
  void putSensorData() {
    ...
    SampleFilter_put(&x, axis_data.x);
    SampleFilter_put(&y, axis_data.y);
    SampleFilter_put(&z, axis_data.z);
  }
};
