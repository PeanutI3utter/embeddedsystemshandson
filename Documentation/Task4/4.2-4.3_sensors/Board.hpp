...
class Board {
public:
  Board(uint8_t WHO_AM_I_VAL, uint8_t SAD, KPS3227SP1C *light_sensor) {
  ...
  virtual void readMagnetometer(uint8_t receive_buffer[]) = 0;

  short readLightSensor() { return light_sensor->read(); };

  void light_sensor_state_transition() { light_sensor->stateTransition(); }
